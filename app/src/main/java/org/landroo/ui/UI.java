...
public class UI 
{
    ...
    public UI(UIInterface context)
    {
    	...
    	for(int i = 0; i < 10; i++) fingers[i] = new Finger();
    }
    ...
    public boolean tapEvent(MotionEvent event)
    {
    	...
    	for(int id = 0; id < event.getPointerCount(); id++)
    	{
			...
			if(id == 0)
			{
				processTap(x, y, event.getAction(), id);
			}
			else if(id == 1)	// secound finger
			{
				processSecTap(x, y, event.getAction(), id);
			}
    ...
    private boolean processTap(float x1, float y1, int action, int id)
    {
        ...
        switch(action) 
        {
        	...
        	case MotionEvent.ACTION_UP:
                ...
                && x1 + TAP_TRESHOLD > lastX && x1 - TAP_TRESHOLD < lastX
                && y1 + TAP_TRESHOLD > lastY && y1 - TAP_TRESHOLD < lastY)
                {
            		// call hold event
                	eventType = 4;
                }
    ...
    private boolean processSecTap(float x1, float y1, int action, int id)
    {
    	...
    	float angle = (float)getAng(x2, y2, x1, y1);
		this.callBack.onRotate(eventMode, x1, y1, angle);
